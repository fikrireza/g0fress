...
@section('title')
  <title>Gofress | Tambah Distribution Maps</title>
@endsection
...
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
      <div class="x_title">
        <h2>Tambah Distribution Maps<small></small></h2>
      ...
      <div class="x_content">
        <form action="{{ route('distribution.store') }}" method="POST" class="form-horizontal form-label-left">
          {{ csrf_field() }}
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <select class="form-control select2_single" name="id_provinsi">
                ...
                @foreach ($getKota as $key)
                  <option value="{{ $key->id }}" {{ $key->id == old('id_provinsi') ? 'selected' : '' }}>{{ $key->nama_kota }}</option>
                @endforeach
              </select>
          ...
          <div class="item form-group {{ $errors->has('nama_kota') ? 'has-error' : ''}}">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <input id="nama_kota" class="form-control" name="nama_kota" placeholder="Contoh: " required="required" type="text" value="{{ old('nama_kota') }}">
