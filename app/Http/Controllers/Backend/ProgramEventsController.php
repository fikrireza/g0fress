...
class ProgramEventsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function() use($request){
          ...
          if (!$image && !$image_thumb) {
            ...
            $img_url = str_slug($request->img_alt,'-').'-932x350 -'.$salt. '.' . $image->getClientOriginalExtension();
            Image::make($image)->fit(932,350)->save('images/programEvent/'. $img_url);

            $img_url_thumb = str_slug($request->img_alt_thumb,'-').'-325x413-'.$salt. '.' . $image_thumb->getClientOriginalExtension();
    ...
    public function edit(Request $request)
    {
        ...
        DB::transaction(function() use($request){
          ...
          elseif($image)
          {
            $img_url = str_slug($request->img_alt,'-').'-923x350 -'.$salt. '.' . $image->getClientOriginalExtension();
            Image::make($image)->fit(932,350)->save('images/programEvent/'. $img_url);
          ...
          elseif($image_thumb)
          {
            $img_thumb = str_slug($request->img_alt_thumb,'-').'-325x413 -'.$salt. '.' . $image_thumb->getClientOriginalExtension();
